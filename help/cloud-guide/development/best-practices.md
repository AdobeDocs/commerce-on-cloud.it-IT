...
- **Completare le attività post aggiornamento**-
  ...
  - Utilizzare SSH per accedere al server remoto e verificare quanto segue:
    ...
    - Controlla lo stato dell’indicizzatore e reindicizza in base alle esigenze. Vedi [Gestione degli indicizzatori](https://experienceleague.adobe.com/en/docs/commerce-operations/configuration-guide/cli/manage-indexers) nella _Guida alla configurazione_.
    ...
    - Controllare i registri `cron` e la tabella `cron_schedule` nel database di Adobe Commerce per verificare lo stato cron ed eseguire nuovamente i processi cron in base alle esigenze.
      Vedi [Registrazione](https://experienceleague.adobe.com/en/docs/commerce-operations/configuration-guide/cli/configure-cron-jobs#logging) nella _Guida alla configurazione_.
